...
impl LogicalPlan {
    ...
    pub fn schema(&self) -> &DFSchemaRef {
        match self {
            ...
            LogicalPlan::Pivot(Pivot { schema, .. }) => schema,
        }
    }
    ...
    pub fn inputs(&self) -> Vec<&LogicalPlan> {
        match self {
            ...
            | LogicalPlan::Pivot(Pivot { input, .. }) => vec![input],
...
/// Pivot operator for transforming rows into columns.
/// This is used for queries that contain `PIVOT` clauses.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct Pivot {
    /// Input logical plan
    pub input: Arc<LogicalPlan>,
    /// Aggregate expression (e.g., SUM(amount))
    pub aggregate_expr: Expr,
    /// Column whose values become new columns
    pub pivot_column: Column,
    /// List of pivot values (distinct values from pivot column)
...
"#;
        ...
        let expected_pg_json = r#"[
  {
    ...
    /*#[test]
    fn test_pivot_plan() -> Result<()> {
        let schema = Arc::new(DFSchema::new_with_metadata(vec![
            Arc::new(Field::new("empid", DataType::Int32, false)),
            Arc::new(Field::new("amount", DataType::Int32, false)),
            Arc::new(Field::new("quarter", DataType::Utf8, false)),
        ], HashMap::new())?);

        let input_plan = LogicalPlanBuilder::scan_empty(None, schema, None)?.build()?;

        let pivot_plan = LogicalPlanBuilder::from(input_plan)
            .pivot(
                col("amount"),
                Column::from_name("quarter"),
                vec![
                    ScalarValue::Utf8(Some("2023_Q1".to_string())),
                    ScalarValue::Utf8(Some("2023_Q2".to_string())),
                ],
            )?
            .build()?;

        assert_eq!(
            format!("{}", pivot_plan.display_indent()),
            "Pivot: amount FOR quarter IN (Utf8(\"2023_Q1\"), Utf8(\"2023_Q2\"))\n  EmptyRelation"
        );
