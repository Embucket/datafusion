...
///
/// The seed is derived from the level through a strong mixer so recursive
/// repartitioning does not map a parent partition into a single child.
/// (Caught by `recursive_scatter_levels_are_decorrelated`.)
pub(super) fn scatter_random_state(level: usize) -> RandomState {
    let base = 0x53_50_4C_00_C0_FF_EE_00u64 ^ (level as u64);
    RandomState::with_seed(splitmix64(base))
}
...
pub(super) struct SpilledSide {
    pub(super) files: Vec<Arc<dyn SpillFile>>,
...
pub(super) struct SharedSpilledSide {
    pub(super) files: Arc<Vec<Arc<dyn SpillFile>>>,
}
...
struct ChunkedState {
    ...
    probe_files: Vec<Arc<dyn SpillFile>>,
    chunk_index: usize,
}
...
fn shared_build_loader(
    ctx: Arc<HashJoinSpillContext>,
    files: Arc<Vec<Arc<dyn SpillFile>>>,
    k: usize,
    random_state: RandomState,
    join_type: JoinType,
...
mod tests {
    ...
    fn result_fingerprint(batches: &[RecordBatch]) -> (usize, u64) {
        let state = RandomState::with_seed(1);
    ...
    async fn partition_writer_rotates_and_cleans_up_files() {
        ...
        let paths: Vec<std::path::PathBuf> = side
            .files
            .iter()
            .filter_map(|f| f.path().map(std::path::Path::to_path_buf))
            .collect();
        assert_eq!(paths.len(), side.files.len());
        for p in &paths {
            assert!(p.exists());
        }
        drop(side);
    ...
    async fn collect_left_forced_spill_matches_in_memory() {
        ...
        for join_type in [Inner, Left, LeftAnti, Full] {
            ...
            // pool must admit two resident per-k tables at once: K=64 keeps
            // the DF55 hash tables below the 8MB pool limit.
            let runtime = RuntimeEnvBuilder::new()
            ...
            let mut session_config = SessionConfig::default().with_batch_size(4096);
            {
                ...
                exec.enable_hash_join_spill = true;
                exec.hash_join_spill_headroom_bytes = 256 * 1024;
                exec.hash_join_spill_partition_count = 64;
            }
            let ctx = Arc::new(
                TaskContext::default()
                    .with_session_config(session_config)
