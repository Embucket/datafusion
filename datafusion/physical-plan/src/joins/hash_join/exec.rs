...
use crate::joins::utils::{
    reorder_output_after_swap, swap_join_projection, update_hash, OnceAsync, OnceFut,
};
...
pub(crate) const HYBRID_HASH_MAX_PARTITIONS: usize = 1 << 16;
...
static NEXT_HHJ_STREAM_ID: AtomicUsize = AtomicUsize::new(0);
...
pub(super) enum OriginalBuildInput {
    ...
    Spilled {
        _spill_manager: Arc<SpillManager>,
...
impl JoinLeftData {
        ...
        F: FnMut(RecordBatch) -> Result<bool>,
    {
        match &self.original_input {
            ...
            OriginalBuildInput::Spilled { spill_file, .. } => {
                let file = File::open(spill_file.path())?;
                let reader = BufReader::new(file);
                // SAFETY: spill files are generated by DataFusion with validated schema/buffers.
                let mut reader = unsafe {
                    StreamReader::try_new(reader, None)?.with_skip_validation(true)
                };
...
impl HashJoinExec {
    ...
    fn compute_properties(
        ...
        let mut output_partitioning = match mode {
            PartitionMode::CollectLeft => Partitioning::UnknownPartitioning(1),
...
impl ExecutionPlan for HashJoinExec {
    ...
    fn execute(
        ...
        let enable_spillable_mode = context
            .session_config()
            .options()
            .optimizer
            .enable_spillable_hash_join;
        let single_stream_mode = matches!(self.mode, PartitionMode::CollectLeft)
            || (self.mode == PartitionMode::PartitionedSpillable && enable_spillable_mode);

        if single_stream_mode && partition > 0 {
            return plan_err!(
                "HashJoinExec in {:?} mode produces a single output partition but partition {partition} was requested. \
                 Insert a RepartitionExec above the join when additional parallelism is required.",
                self.mode
            );
        }
        ...
        let left_fut = match self.mode {
            ...
            PartitionMode::PartitionedSpillable => {
                if !enable_spillable_mode {
                    ...
                    let stream_id =
                        NEXT_HHJ_STREAM_ID.fetch_add(1, Ordering::Relaxed);
                    #[cfg(feature = "hybrid_hash_join_scheduler")]
                    let probe_scheduler_enabled = std::env::var(
                        "DATAFUSION_HHJ_ENABLE_SCHEDULER",
                    )
                    .map(|v| {
                        matches!(
    ...
    let (hashmap, single_batch, left_values, visited_indices_bitmap, original_input) =
        match collect_mode {
            CollectLeftMode::InMemory => {
                ...
                let (hashmap, single_batch, left_values, visited_indices_bitmap) =
                    build_join_data_from_batches(
                        batches_arc.as_slice(),
                        num_rows,
                        &schema,
                        &on_left,
                        &random_state,
            ...
            CollectLeftMode::SpillToDisk { spill_manager } => {
                if num_rows == 0 {
                    ...
                    let spill_file = Arc::new(writer.finish()?.ok_or_else(|| {
                    ...
                    let reloaded_batches = reload_spilled_batches(&spill_file)?;
                    let (hashmap, single_batch, left_values, visited_indices_bitmap) =
                        build_join_data_from_batches(
                            &reloaded_batches,
                            num_rows,
...
fn build_join_data_from_batches(
    batches: &[RecordBatch],
    num_rows: usize,
    schema: &SchemaRef,
    ...
    Ok((hashmap, single_batch, left_values, visited_indices_bitmap))
}

fn reload_spilled_batches(
    spill_file: &Arc<RefCountedTempFile>,
    ...
    // SAFETY: spill files are generated by DataFusion with validated schema/buffers.
    let mut reader =
        unsafe { StreamReader::try_new(reader, None)?.with_skip_validation(true) };
...
mod tests {
    ...
    use crate::{
        common, expressions::Column, metrics::ExecutionPlanMetricsSet,
        repartition::RepartitionExec, stream::RecordBatchStreamAdapter,
    ...
    #[tokio::test]
    async fn collect_left_input_spill_path_rebuilds_state() -> Result<()> {
        ...
        let stream = RecordBatchStreamAdapter::new(
            Arc::clone(&schema),
            stream::iter(vec![
                Ok::<_, DataFusionError>(batch1.clone()),
                Ok::<_, DataFusionError>(batch2.clone()),
            ]),
        );
        let left_stream: SendableRecordBatchStream = Box::pin(stream);

        let metrics_set = ExecutionPlanMetricsSet::new();
        let join_metrics = BuildProbeJoinMetrics::new(0, &metrics_set);
        let pool: Arc<dyn MemoryPool> = Arc::new(UnboundedMemoryPool::default());
        let reservation = MemoryConsumer::new("collect_left_input_spill").register(&pool);
        ...
        let join_data = collect_left_input(
            RandomState::new(),
            left_stream,
        ...
        let expected_rows = batch1.num_rows() + batch2.num_rows();
        assert_eq!(join_data.total_rows(), expected_rows);
        assert_eq!(join_data.batch().num_rows(), expected_rows);
        assert!(!join_data.hash_map().is_empty());

        let mut counted_rows = 0usize;
        join_data.for_each_original_batch(|batch| {
            counted_rows += batch.num_rows();
            Ok(true)
        })?;
        assert_eq!(counted_rows, expected_rows);
        Ok(())
    }
    ...
    async fn join_collect_with_partition_mode(
        ...
        let spillable_enabled = context
            .session_config()
            .options()
            .optimizer
            .enable_spillable_hash_join;
        let single_stream_output = matches!(partition_mode, PartitionMode::CollectLeft)
            || (partition_mode == PartitionMode::PartitionedSpillable && spillable_enabled);
        let requested_partitions = if single_stream_output {
            1
        } else {
            partition_count
        };
        ...
        let mut batches = vec![];
        for i in 0..requested_partitions {
